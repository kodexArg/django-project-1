    ...
    async def notify_clients(self):
        ...
        try:
            ...
            if clients:
                ...
                print("Reload message sent to clients.")
        except asyncio.CancelledError:
            print("Update interrupted, restarting timer...")
...
async def stop_on_keypress():
    """Stop the asyncio event loop on pressing the space bar."""
    loop = asyncio.get_running_loop()
    
    def wait_for_keypress():
        print("Press space bar to stop")
        while True:
            if input().strip().lower() == '':
